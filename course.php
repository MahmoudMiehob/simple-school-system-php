            ...
            <div class="row">
                    ...
                    if ($result->num_rows > 0) { // check if there are results or not
                        ...
                        if ($courses != null) {
                            foreach ($courses as $course) {
                                $sql = "SELECT * FROM students_courses WHERE student_id = ? AND course_id = ?";
                                ...
                                <div class="col">
                                    <div class="card text-center">
                                        ...
                                        <form action="<?php echo $_SERVER['PHP_SELF'] . '?stage=' . $stage ; ?>" method="post">
                                            <input type="hidden" name="course_id" value="<?= $course['id'] ?>">
                                            <input type="hidden" name="teacher_id" value="<?= $course['teacher_id'] ?>">
                                            <?php if(isset($student_id)){
                                                        if ($already_registered) { ?>
                                                            <button class="btn btn-success" disabled>لقد سجلت بالفعل</button>

                                                <?php }else{?>
                                            <button class="btn btn-info" type="submit">سجل الان</button>
                                        <?php }} ?>   
                                        </form>
                ...
                <?php 
                    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                        ...
                        if($stmt->execute()){
                            header('Location: course.php?stage=' . $stage);
                        }
